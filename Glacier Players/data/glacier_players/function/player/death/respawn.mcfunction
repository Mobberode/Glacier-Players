...
$summon armor_stand ~ ~ ~ {CustomName:'{"text":$(respawning_name)}',CustomNameVisible:true,PersistenceRequired:true,Tags:["GlacierPlayer","gp.id$(abnormal_pid_num)","glacier_players.processed_id","GP.DeathProcess"],Silent:true,Invulnerable:true,ShowArms:true}
...
execute as @e[tag=GP.Invs_Fill] run function glacier_players:player/inventory/inventory_initalize
execute as @n[tag=GP.DeathProcess] run function glacier_players:player/death/set_self with storage glacier_players.macro
