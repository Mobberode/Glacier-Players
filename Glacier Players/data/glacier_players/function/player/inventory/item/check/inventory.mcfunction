...
$execute as @e[limit=1,tag=GP.Inventory_Selected.InvSection2] if items entity @s horse.$(player_inv_slot) debug_stick run function glacier_players:player/inventory/item/check/hotbar/override_empty_slot with storage glacier_players.inventory_macro
...
execute unless score @s glacier_players.inventory_slot_slot_number matches 14.. if entity @s[tag=!GlacierPlayer.Inv_Sorted_Item] run function glacier_players:player/inventory/item/check/inventory with storage glacier_players.inventory_macro