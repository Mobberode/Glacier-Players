...
#tellraw @a[tag=expai.debug] ["",{"score":{"name": "@s","objective": "glacier_players.inventory_slot_count"},"color": "gold"},{"score":{"name": "@s","objective": "glacier_players.inventory_slot_comparing_count"},"color": "yellow"}]
...
#execute if score @s glacier_players.inventory_slot_comparing_count >= @s glacier_players.inventory_item_max_stack_size run msg @a[tag=expai.debug] stack limit reached!
