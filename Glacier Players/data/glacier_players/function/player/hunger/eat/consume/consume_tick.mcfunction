...
#Sound every 4 ticks
execute if predicate glacier_players:4_tick_period at @s[tag=!GlacierPlayer.Drinking] run return run playsound entity.eat player @a ~ ~ ~
execute if predicate glacier_players:4_tick_period at @s[tag=GlacierPlayer.Drinking,tag=!GlacierPlayer.Drinking_Honey] run return run playsound entity.drink player @a ~ ~ ~
