...
$execute as $(saved_inventory2_uuid) if items entity @s horse.$(player_inv_slot) $(item_convert_id) run function glacier_players:player/hunger/eat/consume/convert/inventory_sort/success/item_found with storage glacier_players.inventory_macro
...
execute unless score @s glacier_players.inventory_slot_number matches 11.. if score #SortedItem glacier_players.condition matches ..0 run function glacier_players:player/hunger/eat/consume/convert/inventory_sort/hotbar with storage glacier_players.inventory_macro