...
$execute as @e[tag=GP.$(player_inv_id).InvSection3,limit=1] if items entity @s horse.$(player_inv_slot) $(picked_item_id) run function expai_glacier_players:player/inventory/item/check/hotbar/same_item with storage expai_glacier_players.inventory_macro

#Check if current slot is empty
$execute as @e[tag=GP.$(player_inv_id).InvSection3,limit=1] if items entity @s horse.$(player_inv_slot) debug_stick run function expai_glacier_players:player/inventory/item/check/hotbar/override_empty_slot with storage expai_glacier_players.inventory_macro
