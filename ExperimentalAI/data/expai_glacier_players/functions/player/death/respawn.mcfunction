...
$summon armor_stand ~ ~ ~ {CustomName:'{"text":$(respawning_name)}',CustomNameVisible:true,PersistenceRequired:true,Tags:["GlacierPlayer","gp.id$(respawning_id)","expai_glacier_players.processed_id"],Silent:true,Invulnerable:true,ShowArms:true}

$execute as @e[tag=gp.id$(respawning_id),limit=1] run function expai_glacier_players:player/death/set_self with storage expai_glacier_players.macro
