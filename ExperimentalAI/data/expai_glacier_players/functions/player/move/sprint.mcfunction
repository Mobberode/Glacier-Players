...
$execute as @e[tag=m$(marker_num),limit=1,sort=nearest] positioned 0.0 0.0 0.0 run function expai_glacier_players:player/motion/sprint with storage expai_glacier_players.macro
...
$execute at @s run data modify entity @s Motion set from storage expai_glacier_players:motion $(marker_destinations_id)Motion_Sprint
...
tag @s remove expai_glacier_players.sprint